	...
	if (empty($passed_type)) {
		...
		if ($follower_count > 0) {
			...
			while($item = mysqli_fetch_array($follower_timeline, MYSQLI_ASSOC)) {	
				...
				$timeline_added[] = $item['upload_key'];
		...
		$json_output[] = array('status' => $json_status, 'error_code' => 200, 'output' => $timeline_output, 'added' => $timeline_added);
		echo json_encode($json_output);
		exit;
